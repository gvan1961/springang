...
<div class="form-group">
  ...
  <small class="field-help bloqueado" *ngIf="apartamentoBloqueado && apartamentoSelecionado">
    🔒 Apartamento {{ apartamentoSelecionado.numeroApartamento }} selecionado pelo mapa (não pode ser alterado)
  </small>
  
  <small class="field-help" *ngIf="!apartamentoBloqueado && apartamentoSelecionado">
...
ngOnInit(): void {
  ...
  this.route.queryParams.subscribe(params => {
    ...
    // ✅ VERIFICAR SE VEIO DO MAPA (apartamento bloqueado)
    if (params['bloqueado'] === 'true') {
      this.apartamentoBloqueado = true;
      console.log('🔒 Apartamento será bloqueado (veio do mapa)');
    }
 ...
 carregarApartamentos(): void {
  ...
  // ✅ SE VEIO DO MAPA, CARREGAR TODOS (incluindo ocupados)
  // Porque queremos mostrar o apartamento selecionado mesmo que ocupado
  const endpoint = this.apartamentoBloqueado 
    ? 'http://localhost:8080/api/apartamentos'
    : 'http://localhost:8080/api/apartamentos'; // Pode usar getDisponiveis() aqui se preferir

  this.http.get<any[]>(endpoint).subscribe({
    next: (data) => {
      // Se veio do mapa, mostrar apenas o apartamento selecionado
      if (this.apartamentoBloqueado) {
        this.apartamentos = data; // Carregar todos para encontrar o selecionado
      } else {
        // Filtrar apenas disponíveis
        this.apartamentos = data.filter(apt => apt.status === 'DISPONIVEL');
      }
      
      console.log('✅ Apartamentos carregados:', this.apartamentos.length);
      },
      error: (err) => {
